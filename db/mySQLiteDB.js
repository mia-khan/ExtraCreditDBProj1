...
async function getFires() {
  ...
  return await db.all(`SELECT name FROM sqlite_master WHERE type ='table' AND name NOT LIKE 'sqlite_%';`);
}
...
async function createFire(newFire) {
  ...
  for (let i = 0; i < numAttributes; i++) {
      ...
      attributes.push({ name: attributeName, type: attributeType, isPrimaryKey, isForeignKey, referenceTable, referenceField });
